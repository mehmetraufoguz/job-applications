import React, { FC, useState, useEffect, useRef, useMemo, PropsWithChildren } from 'react';
...
const ItemComponent: FC<IsSelected> = ({ isSelected, children }) => (
    isSelected ? <Text color="#00FF00">{children}</Text> : <Text>{children}</Text>
);
...
const StatusComponent: FC<StatusProps> = ({ hasMatch, children, label = '' }) => (
    <Text>{`${label || 'Query'}: `}<Text color="#74BEFF">{children}</Text></Text>
);
...
export interface QuickSearchInputProps {
    ...
    indicatorComponent?: FC<IsSelected>
    itemComponent?: FC<ItemProps>
    highlightComponent?: FC
    statusComponent?: FC<StatusProps>
}

export const QuickSearchInput: FC<QuickSearchInputProps> = (props) => {
    const {
    ...
    function handleKeyPress(ch: string, key: KeyPress) {
        ...
        } else if (key.sequence && hasAnsi(key.sequence)) {
    ...
    return (
        <Box key='quicksearch-input' flexDirection='column'>
            <Box key='status-label'>
                <Status label={label ?? ''} hasMatch={visibleItems.length > 0} children={query} />
            </Box>
            {
                visibleItems.length === 0 ?
                    ...
                    visibleItems.map((item) => {
                        ...
                        labelComponent = (
                            <Text>{preMatch}<HighlightComponent>{match}</HighlightComponent>{postMatch}</Text>
                        )
                        return (
...
// Only named export above, no duplicate
